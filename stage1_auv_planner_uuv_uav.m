function varargout = stage1_auv_planner_uuv_uav(varargin)
...
%             d = d + dmat(pRoute(rng(s,2)),pRoute(rng(s,1))); % make it a circle
        ...
        for ic = 1:numTarChargers
            ...
            for jc = charger_location_ind(ic,1):charger_location_ind(ic,2)-1
                time_charger_travel(charger_count) = dmat(new_order_pStation(jc),new_order_pStation(jc+1))/delta_vc; % add here charger distance estimation, consider obstacles
...
%     if minFun < globalMin
        ...
        if showProg
            ...
            num_journey = ceil(num_chargering_period/nSalesmen); 
            x_list = xy(optStations,1); 
            y_list = xy(optStations,2);
 
            
            for num_j = 1:1:num_journey
                x_uav{num_j} = x_list(num_j:3:end);
                y_uav{num_j} = y_list(num_j:3:end);
            end
            
%             % consider start points
%             static_station_x = 8;
%             static_station_y = 1; 
%             
%             for  num_j = 1:1:num_journey
%                 x_uav{num_j}(end+1) = static_station_x;
%                 y_uav{num_j}(end+1) = static_station_y;
%             end           
 
            % consider start points
            dynamic_station_x = [1 6 13];
            dynamic_station_y = [12 7 1]; 
            
            for  num_j = 1:1:num_journey
                x_uav{num_j}(end+1) = dynamic_station_x(num_j);
                y_uav{num_j}(end+1) = dynamic_station_y(num_j);
            end    
            ...
            for  num_j = 1:1:num_journey
                x_list = x_uav{num_j};
                y_list = y_uav{num_j};
                [trajectory_x,trajectory_y,journey_dist] = TSP_Solver_Matlab(x_list, y_list);
                j_dist(num_j) = journey_dist;
                
                edges_num = height(trajectory_x);
                
                for edg_k = 1:1:edges_num
                    x = trajectory_x(edg_k,:);
                    y = trajectory_y(edg_k,:);
                    if edg_k == 1
                        plot(hAx,x,y,'--O','Color', line_color{num_j},'linewidth', 2);
                    else
                        plot(hAx,x,y,'--O','Color', line_color{num_j},'HandleVisibility', 'off','linewidth', 2);
                    end
                    hold(hAx,'on');
                end
            end 
            
            plot(hAx,dynamic_station_x,dynamic_station_y, ':','Color','k','linewidth', 2)
            
            legend('AUV#1','AUV#2','AUV#3','AUV surfacing points','j=1','j=2','j=3','moving station')
            title(hAx, sprintf('UAV Distance J1=%1.2f + J2=%1.2f + J3=%1.2f', j_dist(1), j_dist(2),j_dist(3)));
%             x_uav1 = x_list(1:3:end);
%             x_uav2 = x_list(2:3:end);
%             x_uav3 = x_list(3:3:end);
%                          
%             y_uav1 = y_list(1:3:end);
%             y_uav2 = y_list(2:3:end);
%             y_uav3 = y_list(3:3:end);
